...
from rag_pipeline.config import DATABASE_URL_LLM
...
- related_team_codes : liste des équipes détectées
- related_player_names : liste des joueurs détectés
...
- Ne jamais utiliser INSERT, UPDATE, DELETE, DROP, ALTER ou TRUNCATE
...
- Utiliser les noms exacts des colonnes et tables ci-dessus
"""
...
def generate_sql_query(question: str) -> str:
    """
    Version simple :
    → pour l’instant on ne met pas encore le LLM
    → on retourne un exemple ou fallback
    """

    q = question.lower()

    if "points" in q or "score" in q:
        return FEW_SHOTS[0]["sql"]

    if "offensive rating" in q:
        return FEW_SHOTS[1]["sql"]

    # fallback simple
    return "SELECT player_name FROM players LIMIT 10;"
...
if __name__ == "__main__":
    ...
    setup_logging()
    print("Test sql_tool lancé")

    result = sql_tool("Qui sont les meilleurs scoreurs ?")
    print(result)